...
- **Snap Configure Hook (Python)**: One-way sync from local snap config → cluster DB
- **Daemon Watcher (Go)**: Background goroutine that polls cluster DB every 5 seconds and syncs to all nodes' snap configs

**Data Flow:**
```
User: snap set openstack feature.X=true (any node)
  ↓
...
* **`sunbeam/provider/maas/deployment.py`**: `ROLETAG_GATES` for MAAS deployment role tags
