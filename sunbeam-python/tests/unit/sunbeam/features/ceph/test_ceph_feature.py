...
class TestCephFeature:
    ...
    def test_run_disable_plans(self, _mock_jhelper, mock_run_plan):
        ...
        with (
            ...
            patch.object(feature, "update_feature_info") as mock_update_info,
        ):
            mock_backend = Mock()
            mock_backend.tfplan = "storage-backend-plan"
        ...
        # The ceph_disabling marker is set before phase 1 and cleared
        # after phase 3.
        assert mock_update_info.call_args_list[0] == call(
            client, {"ceph_disabling": "true"}
        )
        assert mock_update_info.call_args_list[-1] == call(
            client, {"ceph_disabling": "false"}
        )

    @patch.object(ceph_feature, "run_plan")
    @patch.object(ceph_feature, "JujuHelper")
    @patch.object(ceph_feature, "click", Mock())
    def test_run_disable_plans_does_not_clear_marker_on_failure(
        self, _mock_jhelper, mock_run_plan
    ):
        """Marker survives across retries when a phase raises.

        If a phase raises, the ceph_disabling marker stays set so the
        next retry still short-circuits is_internal_ceph_enabled_feature_aware
        and the retry re-enters each idempotent phase safely.
        """
        deployment = Mock()
        deployment.openstack_machines_model = "openstack"
        deployment.get_manifest.return_value = Mock()
        ...
        # Raise on the last phase (phase 3 destroy microceph).
        def fake_run_plan(plan, *_args, **_kwargs):
            if mock_run_plan.call_count >= 4:
                raise RuntimeError("phase 3 failure")

        mock_run_plan.side_effect = fake_run_plan
        ...
        # Marker was set at the top.
        assert mock_update_info.call_args_list[0] == call(
            client, {"ceph_disabling": "true"}
        )
        # Marker is NOT cleared because phase 3 raised.
        for call_args in mock_update_info.call_args_list:
            assert call_args != call(client, {"ceph_disabling": "false"})
    ...
    def test_enable_default_storage_reconciles_when_mode_requires_internal_ceph(
        ...
        mock_run_enable_plans.assert_called_once_with(
            deployment, ANY, False, provider_kwargs={}
        )
...
class TestCephFeatureGetBootstrapDeploySteps:
    """Tests for CephFeature.get_bootstrap_deploy_steps.

    These steps are inlined into provider bootstrap / join plans to
    guarantee that the microceph offer exists before
    DeployControlPlaneStep reads data.juju_offer.microceph.
    """
    ...
    @patch.object(ceph_feature, "JujuHelper")
    def test_returns_empty_when_disabled(self, _mock_jhelper):
        feature = ceph_feature.CephFeature()
        steps = feature.get_bootstrap_deploy_steps(
    ...
    @patch.object(ceph_feature, "JujuHelper")
    def test_returns_empty_when_no_storage_expected(self, _mock_jhelper):
        """Must return [] when no storage node is expected.

        Mirrors MicrocephProvider.get_control_plane_tfvars returning
        ...
        steps = feature.get_bootstrap_deploy_steps(
            self._make_deployment(),
            enabled=True,
            expect_storage_node=False,
        )
        assert steps == []

    @patch.object(ceph_feature, "JujuHelper")
    def test_includes_terraform_init_and_microceph_deploy(self, _mock_jhelper):
        feature = ceph_feature.CephFeature()
        steps = feature.get_bootstrap_deploy_steps(
            self._make_deployment(),
            enabled=True,
        ...
        assert len(steps) == 2
        assert isinstance(steps[0], TerraformInitStep)
        assert isinstance(steps[1], DeployMicrocephApplicationStep)

    @patch.object(ceph_feature, "JujuHelper")
    def test_appends_osd_step_when_node_name_given(self, _mock_jhelper):
        feature = ceph_feature.CephFeature()
        steps = feature.get_bootstrap_deploy_steps(
            self._make_deployment(),
            enabled=True,
        ...
        assert len(steps) == 3
        assert isinstance(steps[2], ConfigureMicrocephOSDStep)
