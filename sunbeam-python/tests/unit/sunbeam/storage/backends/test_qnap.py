...
class TestQnapBackend(BaseBackendTests):
    ...
    def test_config_has_required_fields(self, backend):
        """Test that QNAP config has required fields."""
    ...
    def test_sensitive_fields_are_marked_secret(self, backend):
        """Test that QNAP SAN credentials are marked as secret."""
...
class TestQnapConfigValidation:
    """Test QNAP config validation behavior."""

    def test_protocol_rejects_invalid_value(self, qnap_backend):
        """Test that protocol rejects values other than fc/iscsi."""
        config_class = qnap_backend.config_type()
    ...
    def test_protocol_accepts_iscsi(self, qnap_backend):
