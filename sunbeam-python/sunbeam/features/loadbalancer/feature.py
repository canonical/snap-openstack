...
def _csr_common_name(csr_pem: str) -> str:
    ...
    try:
        ...
        attrs = csr.subject.get_attributes_for_oid(_x509.NameOID.COMMON_NAME)
...
class LoadbalancerFeatureConfig(FeatureConfig):
    ...
    lb_mgmt_cidr: str = "172.31.0.0/24"
    ...
    return {
        ...
        _AUTOCREATE_NETWORK_KEY: questions.ConfirmQuestion(
            ...
            description=(
                ...
                " subnet automatically using the CIDR below. Disable this if you"
                " already have a suitable network and want to provide its IDs."
            ),
        ),
        ...
        _SECGROUPS_KEY: questions.PromptQuestion(
            "Security group IDs for Amphora ports (space-separated)",
            description=(
                "Space-separated list of Neutron security group IDs applied to"
                " Amphora VM ports. Only asked when not auto-creating security groups."
            ),
        ),
    }
...
class AmphoraConfigStep(BaseStep):
    ...
    def prompt(
        ...
        if autocreate_secgroups:
            ...
            raw_secgroups = bank.lb_mgmt_secgroup_ids.ask() or ""
            secgroup_ids = (
                [s.strip() for s in raw_secgroups.split() if s.strip()]
                if isinstance(raw_secgroups, str)
                else raw_secgroups
            )
            if not secgroup_ids:
                raise click.ClickException(
                    "At least one security group ID is required when automatic "
                    "security group creation is disabled."
                )
            variables[_SECGROUPS_KEY] = secgroup_ids
    ...
    def run(self, context: StepContext) -> Result:
        ...
        try:
            self.k8s_tfhelper.update_tfvars_and_apply_tf(
                ...
                override_tfvars={"k8s_config": k8s_app_config},
            )
        except (TerraformException, TerraformStateLockedException) as e:
...
class CreateAmphoraResourcesStep(BaseStep):
    ...
    def run(self, context: StepContext) -> Result:
        ...
        try:
            self.tfhelper.update_tfvars_and_apply_tf(
                ...
                override_tfvars={
                    ...
                    # Only used when create-lb-mgmt-network = true
                    "lb-mgmt-cidr": variables.get(_NETWORK_CIDR_KEY, "172.31.0.0/24"),
        ...
        if not variables.get(_SECGROUPS_KEY):
            ...
            variables[_SECGROUPS_KEY] = [sg for sg in [mgmt_sg, health_sg] if sg]
...
class LoadbalancerFeature(OpenStackControlPlaneFeature):
    ...
    def requires(self) -> set[FeatureRequirement]:  # type: ignore[override]
        """Require Barbican (secrets) when the Amphora feature gate is enabled."""
        if is_feature_gate_enabled("feature.loadbalancer-amphora"):
    ...
    def preseed_questions_content(self) -> list:
        ...
        content = questions.show_questions(
            ...
            section="config",
            section_description=(
                "Amphora provider config."
                " Set autocreate_image/autocreate_network/autocreate_securitygroups"
                " to control which resources Terraform creates automatically."
                " When autocreate_network is true, supply lb_mgmt_cidr;"
                " when false, supply lb_mgmt_network_id and lb_mgmt_subnet_id."
                " When autocreate_securitygroups is false, supply lb_mgmt_secgroup_ids."
            ),
            comment_out=True,
        )
        cert_bank = questions.QuestionBank(
        ...
        if not amphora_enabled:
            ...
            plan: list[BaseStep] = [
                ...
                UpdateCiliumCNIExclusiveStep(
                    deployment, k8s_tfhelper, self.manifest, enable_multus=False
                ),
