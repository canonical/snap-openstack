...
class TelemetryFeature(OpenStackControlPlaneFeature):
    ...
    def _has_metrics_storage(self, deployment: Deployment) -> bool:
        ...
        try:
            ...
            return bool(
                ...
                and is_internal_ceph_enabled(client)
            )
        except ConfigItemNotFoundException:
            return False
    ...
    def run_enable_plans(
        ...
        client = deployment.get_client()
        storage_backend: StorageBackendBase = StorageBackendBase()  # type: ignore[type-arg]
        storage_backend.register_terraform_plan(deployment)
        tfhelper_storage = deployment.get_tfhelper(storage_backend.tfplan)
    ...
    def enable_cmd(
        ...
        elif not is_internal_ceph_enabled(client):
