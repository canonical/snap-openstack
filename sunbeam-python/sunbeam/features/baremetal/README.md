...
```bash
sunbeam baremetal switch-config add netconf|generic NAME --config CONFIGFILE  [--additional-file <file path>]
```

An existing switch configuration can be updated with the command:

```bash
sunbeam baremetal switch-config update netconf|generic NAME --config CONFIGFILE  [--additional-file <file path>]
```
