...
INTERNAL_CEPH_BACKEND_NAME = "internal-ceph"

# Feature-info key used by CephFeature.run_disable_plans to signal that
# a multi-phase disable flow is in progress.  Readers of
# is_internal_ceph_enabled_feature_aware treat this as "not enabled" so
# they do not observe the transient mode/feature disagreement between
# phase 1 (backend destroyed) and phase 3 (microceph destroyed).
CEPH_DISABLING_KEY = "ceph_disabling"
...
def is_internal_ceph_enabled(client: Client) -> bool:
    """Check whether deployment mode requires Sunbeam-managed internal Ceph.

    This is a desired-state answer derived from ``CephConfig.mode``. It does
    not imply that the Ceph feature lifecycle has already been reconciled.

    :param client: the Sunbeam clusterd client
    :return: True if the deployment mode is not NONE (or unset, defaulting
    ...
    return mode != CephDeploymentMode.NONE


def set_ceph_feature_enabled_state(
    deployment: Deployment, client: Client, enabled: bool
) -> None:
...
def ensure_default_ceph_feature(
    deployment: Deployment, show_hints: bool, **kwargs: Any
) -> None:
    """Ensure the default ceph feature is enabled for storage."""
    feature = deployment.get_feature_manager().resolve_feature("ceph")
    if feature is None:
        raise RuntimeError("Failed to resolve ceph feature for default storage.")
    feature_api: Any = feature
    feature_api.enable_default_storage(deployment, show_hints, **kwargs)


def get_default_ceph_bootstrap_steps(
    ...
    *,
    ...
    """Return bootstrap microceph deploy steps from the default ceph feature.

    This is the entrypoint provider commands use to pre-deploy microceph
    (and, for local, configure OSDs) before DeployControlPlaneStep runs
    during bootstrap or first-storage-node join.

    Returns [] if the ceph feature cannot be resolved or does not expose
    ``get_bootstrap_deploy_steps`` — callers can rely on that as a safe
    no-op when a future provider type does not need inline setup.

    See ``CephFeature.get_bootstrap_deploy_steps`` for the flag semantics.
    """
    feature = deployment.get_feature_manager().resolve_feature("ceph")
    if feature is None or not hasattr(feature, "get_bootstrap_deploy_steps"):
        return []
    feature_api: Any = feature
    return feature_api.get_bootstrap_deploy_steps(
        deployment,
        enabled=enabled,
        expect_storage_node=expect_storage_node,
        node_name=node_name,
        accept_defaults=accept_defaults,
    )


def is_internal_ceph_enabled_feature_aware(
    deployment: Deployment, client: Client
) -> bool:
    """Return whether Sunbeam-managed internal Ceph is active.

    Checks both the deployment mode and the ceph feature's enabled state.
    Falls back to the deployment mode if the feature state cannot be read.

    Returns False while a phased disable flow is in progress (signalled
    via ``CEPH_DISABLING_KEY`` on the ceph feature info).  This avoids the
    transient window between phase 1 (backend destroyed) and phase 3
    (microceph destroyed) where the ``enabled`` flag and ``mode`` would
    otherwise disagree.
    """
    from sunbeam.features.interface.v1.base import EnableDisableFeature

    mode_enables_internal_ceph = is_internal_ceph_enabled(client)
    try:
        feature = deployment.get_feature_manager().resolve_feature("ceph")
        if isinstance(feature, EnableDisableFeature):
            feature_info = feature.get_feature_info(client)
            if feature_info.get(CEPH_DISABLING_KEY, "false").lower() == "true":
                return False
        ...
        return mode_enables_internal_ceph

    return ceph_feature_enabled or mode_enables_internal_ceph
