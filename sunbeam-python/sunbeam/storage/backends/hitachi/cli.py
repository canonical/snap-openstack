...
"""CLI functionality for Hitachi storage backend.
...
class HitachiCLI:
    ...
    def __init__(self, backend: HitachiBackend):
    ...
    def register_add_cli(self, add: click.Group) -> None:  # noqa: C901
        ...
        help_text = (
            ...
            "  sunbeam storage add hitachi --name myhitachi --san-ip 10.0.0.10 "
            "--protocol FC\n"
        )
        cmd = click.Command(
    ...
    def register_cli(  # noqa: C901
        ...
        @click.pass_context
        def remove_hitachi(ctx, backend_name: str, yes: bool):
        ...
        remove.add_command(remove_hitachi)

        @click.command(name=self.backend.name)
        @click.argument("backend_name", type=str)
        @click.pass_context
        def config_show_hitachi(ctx, backend_name: str):
            service = self.backend._get_service(deployment)
            config = service.get_backend_config(backend_name, self.backend.name)
            self.backend.display_config_table(backend_name, config)

        config_show.add_command(config_show_hitachi)
        ...
        @click.pass_context
        def config_options_hitachi(ctx, backend_name: str | None = None):
            self.backend.display_config_options()

        config_options.add_command(config_options_hitachi)
