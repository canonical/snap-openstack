...
class StorageBackendRegistry:
    ...
    def _load_backends(self) -> None:
        ...
        for path in sunbeam_storage_backends.iterdir():
            ...
            try:
                ...
                mod = importlib.import_module(f"sunbeam.storage.backends.{backend_name}.backend")
    ...
    def _register_add_commands(self, storage_group: click.Group, deployment: Deployment) -> None:
        ...
        @click.argument("config_args", nargs=-1)  # Accept variable number of key=value arguments
        @click.pass_context
        def add(ctx, backend_type: str, config_args: tuple):
            ...
            Inline configuration:
              sunbeam storage add hitachi name=my-hitachi serial=12345 pools=pool1,pool2 san_ip=192.168.1.100 san_password=secret
            """
            try:
                ...
                for arg in config_args:
                    if "=" not in arg:
                        raise click.BadParameter(f"Configuration argument '{arg}' must be in key=value format")
                ...
                if not config_args:
                    console.print(f"[blue]Setting up {backend.display_name} backend[/blue]")
                    ...
                    if not backend_name:
                        raise click.BadParameter("Backend name is required. Use: name=<backend-name>")
                    ...
                    try:
                        ...
                        fields = getattr(config_class, 'model_fields', None) or getattr(config_class, '__fields__', {})
                        for field_name, field in fields.items():
                            is_required = getattr(field, 'is_required', lambda: getattr(field, 'required', False))()
                            required_text = " (required)" if is_required else ""
                            description = getattr(field, 'description', None) or 'No description'
                            console.print(f"  {field_name}{required_text}: {description}")
    ...
    def _register_remove_commands(self, storage_group: click.Group, deployment: Deployment) -> None:
        ...
        def remove(ctx, backend_type: str, backend_name: str, yes: bool):
            ...
            if not yes:
                click.confirm(f"Remove {backend.display_name} backend '{backend_name}'?", abort=True)
    ...
    def _register_list_commands(self, storage_group: click.Group, deployment: Deployment) -> None:
    ...
    def _register_config_commands(self, storage_group: click.Group, deployment: Deployment) -> None:
        ...
        def show(ctx, backend_type: str, backend_name: str):
            ...
            # Get the backend to access its config class
            backend = self.get_backend(backend_type)
            config_class = backend.config_class
            
            # Create a beautiful table
            table = Table(
                title=f"Configuration for {backend.display_name} backend '{backend_name}'",
                show_header=True,
                header_style="bold blue",
            ...
            if hasattr(config_class, '__fields__'):
                ...
                for field_name, field_info in config_class.__fields__.items():
                    if hasattr(field_info, 'field_info') and hasattr(field_info.field_info, 'description'):
                        field_descriptions[field_name] = field_info.field_info.description or "No description available"
                    elif hasattr(field_info, 'description'):
                        field_descriptions[field_name] = field_info.description or "No description available"
            elif hasattr(config_class, 'model_fields'):
                # Pydantic v2 style
                for field_name, field_info in config_class.model_fields.items():
                    field_descriptions[field_name] = getattr(field_info, 'description', "No description available")
            ...
            for key, value in sorted_config:
                ...
                # Get description from config class
            ...
            if not config:
                console.print(f"[yellow]No configuration found for {backend_type} backend '{backend_name}'[/yellow]")
            else:
                console.print(table)
                console.print(f"[green]✅ Configuration displayed for {backend.display_name} backend '{backend_name}'[/green]")
        ...
        @click.pass_context
        def set(ctx, backend_type: str, backend_name: str, config_pairs: tuple):
            ...
            for pair in config_pairs:
                if "=" not in pair:
                    raise click.BadParameter(f"Invalid config pair: {pair}. Use key=value format.")
                key, value = pair.split("=", 1)
                config_updates[key] = value
            
            # Get deployment and update backend configuration
            deployment = ctx.obj
            backend = self.get_backend(backend_type)
            
            try:
                # Use the backend's update config step
                ...
                console.print(f"[green]✅ Configuration updated for {backend.display_name} backend '{backend_name}'[/green]")
        ...
        def reset(ctx, backend_type: str, backend_name: str, keys: tuple):
            ...
            try:
                # Use the backend's update config step with reset keys
                from sunbeam.core.terraform import TerraformInitStep
                from sunbeam.core.common import run_plan
                ...
                console.print(f"[green]✅ Configuration reset for {backend.display_name} backend '{backend_name}'[/green]")
        ...
        @click.pass_context
        def options(ctx, backend_type: str, backend_name: str = None):
            """List available configuration options for backend."""
            backend = self.get_backend(backend_type)
            console.print(f"[blue]Available configuration options for {backend.display_name}:[/blue]")
            ...
            if hasattr(config_class, '__fields__'):
                ...
                for field_name, field_info in config_class.__fields__.items():
                    ...
                    try:
                        ...
                        if hasattr(field_info, 'default'):
                            default_value = str(field_info.default) if field_info.default is not ... else "Required"
                        else:
                ...
                console.print("  Configuration options are managed dynamically via Terraform.")
                console.print("  Use 'sunbeam storage config show' to see current configuration.")
        
        config.add_command(show)
        config.add_command(set)
        config.add_command(reset)
        config.add_command(options)
