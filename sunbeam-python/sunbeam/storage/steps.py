...
class BaseStorageBackendDeployStep(BaseStep, ABC):
    ...
    def run(self, status: Status | None = None) -> Result:
        ...
        try:
            ...
            try:
                current_tfvars = read_config(self.client, self.backend_instance.tfvar_config_key)
                current_backends = current_tfvars.get("hitachi_backends", {}) if current_tfvars else {}
            except Exception:
                current_backends = {}
            ...
            console.print(f"✅ Successfully deployed {self.backend_instance.display_name} backend '{self.backend_name}'")
            return Result(ResultType.COMPLETED)
            
        except Exception as e:
            LOG.error(f"Failed to deploy {self.backend_instance.display_name} backend {self.backend_name}: {e}")
...
class BaseStorageBackendDestroyStep(BaseStep, ABC):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            f"Destroying {backend_instance.display_name} storage backend {backend_name}",
        )
        self.deployment = deployment
        self.client = client
        self.tfhelper = tfhelper
    ...
    def should_destroy_all_resources(self) -> bool:
        ...
        """
        try:
            current_config = read_config(self.client, self.backend_instance.tfvar_config_key)
            
            # Check both new format (backend-specific keys) and legacy format
            backend_key = f"{self.backend_instance.name}_backends"  # e.g., "hitachi_backends"
            
            if backend_key in current_config:
                backends = current_config[backend_key]
            else:
                raise BackendNotFoundException(f"Backend '{self.backend_name}' not found")
           
            
            # Remove this backend from the count
            backends_without_current = {k: v for k, v in backends.items() if k != self.backend_name}
    ...
    def run(self, status: Status | None = None) -> Result:
        ...
        try:
            ...
            try:
                ...
                if backend_key in current_config and self.backend_name in current_config[backend_key]:
                ...
                if backend_found:
                    ...
                    if self.should_destroy_all_resources():
                        ...
                        # After destroying everything, update config to remove this backend
                        update_config(self.client, self.backend_instance.tfvar_config_key, current_config)
                    else:
            ...
            console.print(f"✅ Successfully removed {self.backend_instance.display_name} backend '{self.backend_name}'")
            return Result(ResultType.COMPLETED)
            
        except Exception as e:
            LOG.error(f"Failed to destroy {self.backend_instance.display_name} backend {self.backend_name}: {e}")
            return Result(ResultType.FAILED, str(e))
...
class BaseStorageBackendConfigUpdateStep(BaseStep, ABC):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            f"Updating {backend_instance.display_name} storage backend configuration for {backend_name}",
        )
        self.deployment = deployment
        self.backend_instance = backend_instance
        self.backend_name = backend_name
    ...
    def pre_update_hook(self, status: Status | None = None) -> Result:
        """Hook called before configuration update. Override for custom pre-update logic."""
        return Result(ResultType.COMPLETED)
    
    def post_update_hook(self, status: Status | None = None) -> Result:
        """Hook called after configuration update. Override for custom post-update logic."""
        return Result(ResultType.COMPLETED)
        ...
        if "charm_config" in backend_config:
            ...
            for key in reset_keys:
                if default_instance and hasattr(default_instance, key):
                    ...
                    field_info = config_class.__fields__.get(key)
    ...
    def run(self, status: Status | None = None) -> Result:
        ...
        try:
            ...
            try:
                ...
                if backend_key not in current_config or self.backend_name not in current_config[backend_key]:
                    return Result(ResultType.FAILED, f"Backend {self.backend_name} not found")
                ...
                console.print(f"✅ Successfully {operation_type}d {self.backend_instance.display_name} backend '{self.backend_name}' configuration")
                return Result(ResultType.COMPLETED)
            ...
            LOG.error(f"Failed to update {self.backend_instance.display_name} backend {self.backend_name} configuration: {e}")
            return Result(ResultType.FAILED, str(e))
