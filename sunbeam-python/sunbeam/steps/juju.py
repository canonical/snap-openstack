...
class JujuGrantModelAccessStep(BaseStep, JujuStepHelper):
    ...
    def run(self, context: StepContext) -> Result:
        ...
        try:
            ...
            LOG.exception(
                "Error granting user %s admin access on model %s",
                self.username,
                self.model,
            )
            return Result(ResultType.FAILED, str(e))
        ...
        try:
            ...
            with open(log_file, "wb+") as f:
                ...
                while True:
                    ...
                    LOG.debug(
                        "Juju registraton: expect got regex related to %r",
                        expect_list[index],
                    )
                    if index in (0, 1, 3):
            ...
            LOG.exception("Error registering user %s in Juju", self.username)
            return Result(ResultType.FAILED, str(e))
...
class AddJujuMachineStep(BaseStep, JujuStepHelper):
    ...
    def run(self, context: StepContext) -> Result:
        ...
        try:
            ...
            LOG.exception("Error adding machine %s to Juju", self.machine_ip)
