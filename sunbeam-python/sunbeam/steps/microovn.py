...
class SetOvnProviderStep(BaseStep):
    ...
    def get_config_from_snap(self, snap: Snap) -> ovn.OvnProvider:
        ...
        """
        try:
            value = snap.config.get(ovn.SNAP_PROVIDER_CONFIG_KEY)
            return ovn.OvnProvider(value)
        except UnknownConfigKey:
            return ovn.DEFAULT_PROVIDER
        except ValueError:
            raise ValueError(f"Invalid OVN provider value in snap config: {value}")
