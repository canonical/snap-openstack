...
from sunbeam.features.microceph.steps import DeployMicrocephApplicationStep
...
class LatestInChannelCoordinator(UpgradeCoordinator):
    ...
    def get_plan(self) -> list[BaseStep]:
        ...
        storage_backend: StorageBackendBase = StorageBackendBase()
        storage_backend.register_terraform_plan(self.deployment)
        storage_tfhelper = self.deployment.get_tfhelper(storage_backend.tfplan)
        plan.extend(
            [
