...
from sunbeam.features.ceph.microceph import ceph_replica_scale
...
class BackfillCephFeatureStateStep(BaseStep):
    ...
    def run(self, context: StepContext) -> Result:
        ...
        # Persist CephConfig.mode explicitly so that any future strict
        # equality comparison (config.mode == CephDeploymentMode.MICROCEPH)
        # works on upgraded clusters where the mode was never written.
        config = load_ceph_config(self.client)
        if config.mode is None:
            config.mode = CephDeploymentMode.MICROCEPH
...
class ImportCephResourcesToStorageFrameworkStep(BaseStep):
    ...
    def run(self, context: StepContext) -> Result:
        ...
        # Once imports succeed, the legacy-id map in clusterd has served
        # its purpose and should be removed so it does not linger past
        # the migration window.
        try:
            self.client.cluster.delete_config(STORAGE_BACKEND_LEGACY_IMPORT_IDS_KEY)
        except ConfigItemNotFoundException:
            pass
...
class MigrateCinderVolumeToStorageFrameworkStep(BaseStep):
    ...
    def run(self, context: StepContext) -> Result:
        ...
        Ordering is critical for retry safety: every clusterd write is
        idempotent and happens BEFORE the irreversible old-state clear.
        Any failure up to the final clear leaves the old plan state
        intact, so `is_skip` will re-run the whole flow on retry.
        """
        model_info = self.jhelper.get_model(self.model)
        model_uuid = model_info["model-uuid"]

        # 1. Capture legacy IDs from the old state (read-only).
        try:
        ...
        # 2. Register internal-ceph in clusterd (idempotent).
        try:
        ...
        # 3. Populate TerraformVarsStorageBackends (idempotent merge).
        try:
        ...
        # 4. Clear the old plan state LAST — this is irreversible and
        # is what makes `is_skip` return SKIPPED on subsequent runs.
        try:
